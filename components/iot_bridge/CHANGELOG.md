...
- **SPI / SDIO bridge netif**: `esp_bridge_config.h` removes the compile-time `NUM_DEFINED_MACROS` guard and the legacy `BRIDGE_EXTERNAL_NETIF_*` enum; introduces `esp_bridge_netif_type_t` (`IOT_BRIDGE_NETIF_WAN` / `IOT_BRIDGE_NETIF_LAN`). Netif `if_key` values change from `SPI_DEF` / `SDIO_DEF` to **`SPI_LAN` / `SDIO_LAN`** (data forwarding / DHCPS) and **`SPI_WAN` / `SDIO_WAN`** (external / DHCP client). Code or scripts that look up netifs by the old keys must be updated ([98b3309](https://github.com/espressif/esp-iot-bridge/commit/98b33095fed681ffeeb6b3b84484eb9f33ed6816))
- **Kconfig**: SPI and SDIO external vs data-forwarding options are mutually exclusive via `depends on` (replacing long in-menu warnings only); Ethernet external option label/help text shortened (behavior unchanged) ([98b3309](https://github.com/espressif/esp-iot-bridge/commit/98b33095fed681ffeeb6b3b84484eb9f33ed6816))
